# Contact intake (Google Sheet + email)

The contact form at the bottom of the home page posts to a Google Apps Script web app.
It appends one row per message to a Sheet and emails you a notification with the
...
1. Create a Google Sheet in the account that should receive the notifications. Name it
   ...
   formats its own `Applications` tab on the first submission. (The tab keeps that name
   so rows written by the old per-service application forms still line up.)
...
4. **Deploy → New deployment → ⚙ → Web app**, then:
   ...
   - Who has access: **Anyone**  ← must be "Anyone", not "Anyone with a Google account",
     or senders get a login page instead of a submission.
...
- Open the `/exec` URL in a browser. `{"ok":true,"service":"FIRST LIGHT contact
  intake"}` means it is deployed and public. A deployment made before this file was
  reworded still answers `... application intake`, which is fine — see below.
- Submit the form on the site. Within a couple of seconds the button should be replaced
  by "Message received", a row should appear in the Sheet, and the mail should arrive. If
  the button comes back with an error instead, the row was not written — check
  **Executions** in the Apps Script editor for the reason.
...
- **The URL is public.** It has to be, for the forms to reach it. Anyone who reads the
  ...
  field on the form and a matching check in `doPost`.
- **Where the data lives.** In your Drive, under whichever Google account owns the
  Sheet. Senders' names, emails and messages are personal data — keep the Sheet private
  and share it deliberately.
- **The form is useless until `FORM_ENDPOINT` is set.** It refuses to submit and says
  so, rather than pretending a message went through.

## Adding or reordering columns

The live form sends only `name`, `email` and `message`; the other columns are left over
from the retired per-service application forms and stay blank.
